...
import com.mizlicai.eudemon.mng.constraint.Excel;
...
public class ExcelUtil {
            ...
            for(int i=rowTitles+1;i<=rowNum;i++){
                ...
                if(row==null){
				 ...
				 /*if(StringUtils.isEmpty( row.getCell(0).getStringCellValue())){
					 logger.info("第"+i+"行关键值为空。第一列为关键值");
					 continue ;
				 }else*/ if(row.getCell(0).getStringCellValue().startsWith("运单号")){
                        continue;
                    }
                }
                tempOrderbill=c.newInstance();
     ...
     * @descript 读取excel并分装成类
     */
    @SuppressWarnings("unchecked")
    public static Collection<?> importExcel(InputStream inputstream, Class clazz, int title, int sheetNum){
    ...
    private static void createSheet( HSSFWorkbook workbook,String title,String sheetName, Collection<?>
        ...
        getTitle(workbook,titles,sheet,null==map?1:2);
        setValue(sheet,null==map?2:3,styles,dataSet);
    ...
    private static int createHeaderRow(String title, Sheet sheet,
                                       HSSFWorkbook workbook, int fieldWidth) {
